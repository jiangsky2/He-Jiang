...
tau = 0.75;  %  parameter in quantile loss function      
        ...
        for s = 1 : length(lambda_seq)
                ...
                while Err_outer >= eps_outer && M_outer <= M_max_outer
                        ...
                        while Err >= eps && M <= M_max
                                ...
                                m = y_trn_c - H_trn_c_W * beta;
                                % Using quantile loss function
                                p1  = m * (tau - 1) .* (m <0) + (m * tau ) .*  (m>= 0);    
                                wmatrix = diag(p1);
                                epsilon = 1e-2;
...
disp('############################################################################################################################################');
disp(['dataName = ',num2str(dataName), '---NumFeatureInSubset=', num2str(NumFeatureInSubset),'-----RMSE2  = ',num2str(RMSE2), '---tau= ',num2str(tau),'---MaxN=',num2str(M_max),...
